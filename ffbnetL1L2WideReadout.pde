...
int dim=64;
int netWidth=256;// 4 times input dimension
int layerDepth=4;
int mutate=25;
float precision=25f;
int iterations=300;
...
class FFBNet { 
  ...
  FFBNet(int netWidth, int netDepth) {
    ...
    parameters=new float[2*depth*vecLen+vecLen]; //for parameterized nonlinear functions and readout layer
  ...
  void recall(float[] resultVec, float[] inVec) {
    ...
    whtBuffer();  // final WHT filter bank
    int readN=vecLen/resultVec.length;
    int bufferIdx=0;
    for (int i=0; i<resultVec.length; i++) {  // Readout layer
      resultVec[i]=parameters[pIdx++]*buffer[bufferIdx++];
