...
class FFBNet { 
  ...
  FFBNet(int vecLen, int depth) {
    this.vecLen=vecLen;
    this.depth=depth;
    scNext=(float)Math.sqrt(1.0/vecLen); //scaling for 1 WHT 
    scFirst=1.7f*scNext*scNext;
    scNext*=1.7f;
  ...
  void recall(float[] resultVec, float[] inVec) {
    ...
    // sphere data ie. constant vector length
    float adj = scFirst/(float) Math.sqrt((sumsq/vecLen) + 1e-20f);
    for (int i=0; i<vecLen; i++) {
